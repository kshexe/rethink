...
describe(MODEL_ID, () => {
    test('declares power, course and start as writable, plus two read-only diagnostics', () => {
        ...
        assert.equal(components.smart_care_night_care.command_topic, '$this/smart_care_night_care/set')
    ...
    test('smart_care_night_care write reproduces the captured frame byte for byte, both directions', () => {
        for (const [value, want] of [
            ['ON', 'aa0df0e5000201ff011a01ffbb'],
            ['OFF', 'aa0df0e5000201ff011a00fcbb'],
        ] as [string, string][]) {
            const { thinq, dev } = makeDevice()
            thinq.resetRecorder()
            dev.setProperty('smart_care_night_care', value)
            assert.equal(thinq.outbox.length, 1, `smart_care_night_care=${value} sent one frame`)
            assert.equal(thinq.outbox[0].toString('hex'), want, `smart_care_night_care=${value}`)
        }
    })

    test('smart_care_night_care is published optimistically as soon as it is set, not waiting on a device echo', () => {
        const { ha, dev } = makeDevice()
        dev.setProperty('smart_care_night_care', 'ON')
        assert.equal(ha.devices[DEVICE_ID].properties.smart_care_night_care, 'ON')
        dev.setProperty('smart_care_night_care', 'OFF')
        assert.equal(ha.devices[DEVICE_ID].properties.smart_care_night_care, 'OFF')
    })
