...
/**
 * An address is not excluded by HOSTNAME on its own - four dot-separated all-digit labels
 * satisfy it just as well as a real hostname does. Minting a `type: 'dns'` SAN for one would be
 * malformed (an address belongs in a `type: 'ip'` entry instead), and this fork's own
 * advertisedHost() relies on this same function to refuse handing an appliance an address it
 * would store and be pinned to - see cloud/thinq2/provisioning.ts.
 */
export function isPlausibleHostname(name: string) {
    return name.length > 0 && name.length <= 253 && HOSTNAME.test(name) && isIP(name) === 0
}
